...
// @ts-ignore: Decorator
@inline export function serializeString(data: string, out: string | null = null, offset: i32 = 0): string {
  if (data.length === 0) {
    if (out) out += "\"\"";
    return quoteWord + quoteWord;
  }

  let result = out ? createStringSink(out) : new StringSink(quoteWord);
