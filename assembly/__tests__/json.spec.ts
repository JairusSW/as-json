...
describe("AS-JSON Test Suite", () => {
  test("Should (de)serialize strings", () => {
    expect(JSON.stringify<string>('Hello World"')).toStrictEqual(
      '"Hello World\\""'
    );
    expect(JSON.parse<string>('"Hello World\\""')).toStrictEqual(
      'Hello World"'
    );
  });
  ...
  test("Should (de)serialize nulls", () => {
    ...
    expect(JSON.parse<Nullable | null>("null")).toStrictEqual(
      <Nullable | null>null
    );
  });
