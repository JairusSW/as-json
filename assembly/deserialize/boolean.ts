...
@inline export function deserializeBoolean(data: string): ProductValue {
    ...
    if (data.length === 4 && firstChar === tCode && load<u64>(changetype<usize>(data)) === 28429475166421108) return Product.Ok<boolean>(true);
    else if (data.length === 5 && firstChar === fCode && load<u64>(changetype<usize>(data), 2) === 28429466576093281) return Product.Ok<boolean>(false);
    else {
