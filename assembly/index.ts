...
export namespace JSON {
  ...
  * @returns string
  */
  export function stringifySafe<T>(data: T, out: string): Option<string> {
    if (isBoolean<T>()) {
      return Option.Some(serializeBool(data as bool));
    } else if (isInteger<T>()) {
      // @ts-ignore
      return Option.Some(serializeInteger<T>(data));
    } else if (isFloat<T>(data)) {
      // @ts-ignore
      return Option.Some(serializeFloat<T>(data));
      // @ts-ignore: Function is generated by transform
    } else if (isNullable<T>() && changetype<usize>(data) == <usize>0) {
      return Option.Some(NULL_WORD);
      // @ts-ignore
