...
class JSONTransform extends Visitor {
  ...
  visitCallExpression(node, ref) {
    ...
    if (!(node.expression.kind == 21 && node.expression.property.text == "stringifyTo") && !(node.expression.kind == 6 && node.expression.text == "stringifyTo")) return;
    ...
    if (ref.kind == 9) {
      ...
      const nodeIndex = source.statements.findIndex((n) => {
        ...
        if (n.kind == 38 && n.expression == node) return true;
        return false;
      });
