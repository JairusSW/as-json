...
class JSONTransform extends Visitor {
    ...
    visitClassDeclaration(node) {
        ...
        if (!node.members.find((v) => v.name.text == "__SERIALIZE_BS"))
            node.members.push(SERIALIZE_BS_METHOD);
    ...
    visitImportStatement(node) {
        ...
        if (!source)
            return;
        if (source.statements.some(stmt => stmt.kind === 59 &&
            stmt.name.text === "JSON"))
...
function strToNum(data, simd = false, offset = 0) {
    ...
    while (n >= 2) {
        const value = ((data.charCodeAt(offset + 1) << 16) |
            data.charCodeAt(offset + 0));
...
function strToStores(data, simd = false) {
    console.log(data);
    const out = [];
